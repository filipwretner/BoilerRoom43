...
while (keepRunning) {
// Funktion för att visa multiplikationstabell
function showMultiplicationTable() {
    let firstInput = Number(prompt(`Vilket tal vill du visa multiplikationstabellen för?`));
    while (isNaN(firstInput)) {
        firstInput = Number(prompt(`Ogiltig inmatning. Ange ett giltigt nummer:`));
    }

    let secondInput = Number(prompt(`Hur mycket av multiplikationstabellen ska visas? `));
    while (isNaN(secondInput) || secondInput < 1) {
        secondInput = Number(prompt(`Ogiltig inmatning. Ange ett positivt heltal för att visa tabellen:`));
    }

    console.log(`Multiplikationstabell för ${firstInput}:`);
    for (let i = 1; i <= secondInput; i++) {
        console.log(`${firstInput} * ${i} = ${firstInput * i}`);
    }
}

// Anropa funktionen för att visa multiplikationstabell
showMultiplicationTable();



// Huvudloopen för att hålla applikationen igång


    // Din kod här
    // T.ex. anropa andra funktioner eller logik för att hålla applikationen aktiv

    // Fråga användaren om de vill fortsätta
    let continueChoice = prompt("Vill du fortsätta använda applikationen? (ja/nej)").toLowerCase();
    keepRunning = (continueChoice === "ja");
}
